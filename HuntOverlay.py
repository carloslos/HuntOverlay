...
import sys, os, json, ctypes, traceback, shutil, threading, math, time
...
# Reference map art lives in maps/<data.json index>.webp (1.webp .. 4.webp).
#
# The captured region is downscaled to grayscale and split into a grid of tiles.
# Each tile is compared to the matching reference tile with a zero-normalized
# cross correlation (ZNCC), and the map score is the mean of the *best* tiles
# only. Trimming the worst tiles is what makes this survive local damage:
# burn marks, mission markers, clue rings and leftover overlay dots each ruin a
# handful of tiles, and those tiles are simply thrown away instead of dragging
# the whole score down. Per-tile normalization also cancels local brightness and
# contrast differences, so a dimmed or tinted map still matches.

# Side length of the square the images are reduced to before comparison.
MATCH_GRID = 48

# Tiles per axis. MATCH_GRID must divide evenly by this.
MATCH_BLOCKS = 6

# Fraction of the worst scoring tiles discarded before averaging. With 36 tiles
# this drops the 10 most damaged ones, enough for 3 burn marks plus map clutter.
MATCH_TRIM = 0.30

# Tiles flatter than this (grayscale standard deviation) carry no texture to
# match on: solid burn marks, plain water, empty map edges. They are skipped.
MATCH_MIN_TILE_STD = 4.0

# Never average fewer than this many tiles, however much of the map is obscured.
MATCH_MIN_TILES = 8

# Minimum trimmed correlation for a match to be considered confident.
MATCH_THRESHOLD = 0.20

# Minimum lead over the runner-up. A high score that two maps share is ambiguous.
MATCH_MARGIN = 0.05
...
def _gray_grid(qimg):
    """Downscale an image to MATCH_GRID x MATCH_GRID grayscale, row major."""
    if qimg is None or qimg.isNull():
        return None

    img = qimg.convertToFormat(QtGui.QImage.Format_Grayscale8).scaled(
        MATCH_GRID, MATCH_GRID, QtCore.Qt.IgnoreAspectRatio, QtCore.Qt.SmoothTransformation
    )
    if img.isNull():
        return None

    # Read the raw buffer rather than per pixel; rows are padded to bytesPerLine.
    try:
        buf = bytes(img.constBits())
        bpl = int(img.bytesPerLine())
        if len(buf) >= bpl * MATCH_GRID:
            out = []
            for y in range(MATCH_GRID):
                out.extend(buf[y * bpl : y * bpl + MATCH_GRID])
            return out
    except Exception:
        pass

    return [QtGui.qGray(img.pixel(x, y)) for y in range(MATCH_GRID) for x in range(MATCH_GRID)]

def _tile_vectors(qimg):
    """
    Split the downscaled image into MATCH_BLOCKS x MATCH_BLOCKS tiles and return one
    zero-mean, unit-length vector per tile. Featureless tiles become None so the
    matcher can skip them.
    """
    g = _gray_grid(qimg)
    if g is None:
        return None

    t = MATCH_GRID // MATCH_BLOCKS
    n = t * t
    tiles = []

    for by in range(MATCH_BLOCKS):
        for bx in range(MATCH_BLOCKS):
            vals = []
            for y in range(by * t, by * t + t):
                base = y * MATCH_GRID + bx * t
                vals.extend(g[base : base + t])

            mean = sum(vals) / n
            centered = [v - mean for v in vals]
            ss = sum(c * c for c in centered)
            if math.sqrt(ss / n) < MATCH_MIN_TILE_STD:
                tiles.append(None)
                continue
            norm = math.sqrt(ss)
            tiles.append([c / norm for c in centered])

    return tiles

class MapMatcher:
    """
    Holds the reference tile vectors and matches a captured region against them.
    index_to_name maps a data.json map index (e.g. 1) to its map name.
    """
    def __init__(self, index_to_name: dict):
        self.refs = []  # list of (name, tiles)
        d = maps_dir()
        for idx, name in index_to_name.items():
            ...
            if img.isNull():
                continue
            tiles = _tile_vectors(img)
            if tiles is not None:
                self.refs.append((str(name), tiles))

    def available(self) -> bool:
        return len(self.refs) > 0

    def score_all(self, region):
        """Returns [(name, trimmed_score)] sorted best first, or [] if unusable."""
        cap = _tile_vectors(region)
        if cap is None or not self.refs:
            return []

        trim = int(round(MATCH_TRIM * len(cap)))
        out = []

        for name, ref in self.refs:
            scores = []
            for a, b in zip(cap, ref):
            ...
            keep = max(min(len(scores), MATCH_MIN_TILES), len(scores) - trim)
            scores.sort()
            kept = scores[len(scores) - keep :]
    ...
    def detect(self, region):
        """
        Returns (best_map_name, score, margin). margin is the lead over the runner-up;
        a small margin means two maps scored alike and the guess is unreliable.
        Returns (None, 0.0, 0.0) when no reference or region is usable.
        """
        ranked = self.score_all(region)
        if not ranked:
            return None, 0.0, 0.0

        name, score = ranked[0]
        margin = score - ranked[1][1] if len(ranked) > 1 else score
        return name, score, margin
...
class Overlay(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        # Latched while Tab is held down together with Alt/Ctrl/Shift.
        ...
        # System tray setup. The tray is always created so auto-detect map can
        # report its result even when minimize to tray is off.
        self.tray = None
        self._ensure_tray()
        ...
        # Start with the control panel minimized to tray, silently.
        if self.minimize_to_tray:
            self._hide_panel_to_tray(silent=True)
        else:
            self.panel.show()
    ...
    def _hide_panel_to_tray(self, silent: bool = False):
        self._ensure_tray()
        self.panel.hide()
    ...
    def _bind_pressed(self, name: str) -> bool:
        ...
        if vk == 0:
            return False
        if vk == VK_TAB:
            # Once Tab is pressed together with a modifier (Alt+Tab, Ctrl+Tab,
            # optionally Shift+Tab) the press stays blocked until Tab is released,
            # so releasing the modifier first does not leak a bare Tab through.
            if key(VK_CONTROL) or key(VK_MENU) or (self.block_shift_tab and key(VK_SHIFT)):
    ...
    def _grab_map_region(self):
        ...
        if was_visible:
            self.hide()
            # processEvents alone can return before the compositor has repainted,
            # which would capture our own POI dots on top of the map.
    ...
    def _detect_and_switch_map(self):
        ...
        """
        title = tr("Auto-detect map")
        if not self.map_matcher.available():
            self._notify(title, tr("No reference map images found in maps folder"))
            return

        region = self._grab_map_region()
        name, score, margin = self.map_matcher.detect(region)
        if not name:
            self._notify(title, tr("Could not capture the screen"))
            return

        # Low confidence still switches to the best guess but flags it.
        self.switch(name)
        if score >= MATCH_THRESHOLD and margin >= MATCH_MARGIN:
            self._notify(title, tr("Switched to ") + f"{tr(name)}  ({score:.2f})")
        else:
            self._notify(title, tr("Best guess ") + f"{tr(name)}  ({score:.2f} / +{margin:.2f})")
